...
let successMessage = 'Спасибо за заявку. Наш менеджер свяжется с вами в течение 2-х часов.';
let callbackMessage = 'Наш менеджер перезвонит вам в рабочее время';
...
function changeMessageText(instance, status) {
  let statusMsg = '';
  if (status === true) {
    if (((day === 0) || (hour < 9) || (hour >= 19)) || ((day === 6) && (hour >= 15))) {
      statusMsg = callbackMessage;
    } else {
